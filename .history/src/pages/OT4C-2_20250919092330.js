...
const OT4C2 = ({ userData }) => {
  ...
  const fetchDailyStaff = async (rates = {}, currentShiftData = {}, previousShiftData = {}) => {
    try {
      ...
      // แยกพยาบาลและผู้ช่วย
      const nursesList = dailyStaffList.filter(member => {
        const position = member.position || '';
        return position.includes('พยาบาลวิชาชีพ') || position.includes('พยาบาล');
      });
      const assistantsList = dailyStaffList.filter(member => {
        const position = member.position || '';
        return position.includes('ผู้ช่วยพยาบาล') || position.includes('พนักงานผู้ช่วยเหลือคนไข้');
      });
      ...
      const updatedStaff = Array(8).fill().map((_, index) => {
        ...
        if (globalIndex < dailyStaffList.length) {
          const member = dailyStaffList[globalIndex];
  ...
  const updateStaffWithShiftData = async (currentShiftData = {}, previousShiftData = {}) => {
    ...
    const updatedStaff = formData.staff.map((staff, index) => {
      const globalIndex = (currentPage - 1) * staffPerPage + index;
      if (globalIndex < dailyStaff.length) {
        const member = dailyStaff[globalIndex];
