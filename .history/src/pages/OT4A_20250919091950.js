...
const OT4A = ({ userData }) => {
  ...
  const fetchMonthlyStaff = async (rates = {}, shiftData = {}) => {
    try {
      ...
      const currentStaffList = currentStaffType === 'nurses' ? 
        (nursesList.length > 0 ? nursesList : monthlyStaffList.slice(0, Math.ceil(monthlyStaffList.length / 2))) :
        (assistantsList.length > 0 ? assistantsList : monthlyStaffList.slice(Math.ceil(monthlyStaffList.length / 2)));
