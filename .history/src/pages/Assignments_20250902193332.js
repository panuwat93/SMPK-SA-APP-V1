    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <div className="text-xs text-blue-700 space-y-1">
          <div>วันที่เลือก: {formatDate(selectedDate)}</div>
          <div>จำนวนสมาชิกทีม: {teamMembers.length} คน</div>
          <div>ข้อมูลตารางเวร: {scheduleData.schedule ? 'มี' : 'ไม่มี'}</div>
          <div>พยาบาล: {teamMembers.filter(m => m.role === 'พยาบาล').length} คน</div>
          <div>ผู้ช่วย: {teamMembers.filter(m => m.role !== 'พยาบาล').length} คน</div>
        </div>
      </div>
