...
const Dashboard = ({ user }) => {
  ...
  const getStaffShifts = (date) => {
    ...
    // ดึงข้อมูลจาก cellValues format (เหมือนใน ShiftSchedule)
    const topCellKey = `${teamMembers[memberIndex].id}-${dayIndex}-top`;
    const bottomCellKey = `${teamMembers[memberIndex].id}-${dayIndex}-bottom`;
    
    const topShift = scheduleData.cellValues?.[topCellKey] || '';
    const bottomShift = scheduleData.cellValues?.[bottomCellKey] || '';
    ...
    console.log('Shifts for day', dayIndex, ':', { 
      ...
      topCellKey, 
      bottomCellKey,
      memberId: teamMembers[memberIndex]?.id 
    });
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="mb-4 p-4 bg-gray-100 rounded-lg text-sm">
                ...
                {scheduleData && (
                  <div>
                    <p>Cell Values: {scheduleData.cellValues ? 'Available' : 'Not available'}</p>
                    <p>Cell Values Count: {Object.keys(scheduleData.cellValues || {}).length}</p>
                  </div>
                )}
