...
const ShiftSchedule = ({ userData }) => {
  ...
  const changeMonth = (direction) => {
    ...
    // ดึงข้อมูลวันหยุดใหม่เมื่อเปลี่ยนเดือน
    fetchHolidaysFromFirebase();
  };
      ...
      <div className="w-full">
        ...
        <div className="mb-6 bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-gray-200">
          <div className="grid grid-cols-3 items-center mb-4">
            ...
            {/* แสดงข้อความแจ้งเตือนวันหยุด */}
            {holidayMessage.text && (
              <div className={`mt-3 px-4 py-2 rounded-lg text-sm ${
                holidayMessage.type === 'success' ? 'bg-green-100 text-green-800 border border-green-200' :
                holidayMessage.type === 'error' ? 'bg-red-100 text-red-800 border border-red-200' :
