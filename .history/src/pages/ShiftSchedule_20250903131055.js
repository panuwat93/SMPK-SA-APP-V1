...
const ShiftSchedule = ({ userData }) => {
  ...
  const autoAssignAssistantShifts = async () => {
    ...
    try {
      ...
      for (let dayIndex = 0; dayIndex < days.length; dayIndex++) {
        ...
        if (newMorningCount < 1 || newAfternoonCount < 1 || newNightCount < 1) {
  ...
  // จัดเวรทั้งหมดสำหรับวันหนึ่ง (เช้า 1, บ่าย 1, ดึก 1)
  const assignAllShiftsForDay = (dayIndex, shiftCounts) => {
    ...
    // จัดเวรเช้า 1 คน (แถวบน) - สุ่ม ช หรือ ด
    let morningAssigned = 0;
    for (const assistant of availableAssistants) {
      if (morningAssigned >= 1 || usedAssistants.has(assistant.id)) continue;
    ...
    // จัดเวรบ่าย 1 คน (แถวล่าง) - ใส่ บ
    let afternoonAssigned = 0;
    for (const assistant of availableAssistants) {
      if (afternoonAssigned >= 1 || usedAssistants.has(assistant.id)) continue;
    ...
    // จัดเวรดึก 1 คน (แถวล่าง) - ใส่ ด
    let nightAssigned = 0;
    for (const assistant of availableAssistants) {
      if (nightAssigned >= 1 || usedAssistants.has(assistant.id)) continue;
  ...
  const assignRemainingShifts = (dayIndex, currentMorningCount, currentAfternoonCount, currentNightCount, shiftCounts) => {
    ...
    // จัดเวรเช้าเพิ่มจนครบ 1 คน
    while (currentMorningCount < 1) {
      let assigned = false;
      for (const assistant of availableAssistants) {
    ...
    // จัดเวรบ่ายเพิ่มจนครบ 1 คน
    while (currentAfternoonCount < 1) {
      let assigned = false;
      for (const assistant of availableAssistants) {
        if (usedAssistants.has(assistant.id)) continue;
    ...
    // จัดเวรดึกเพิ่มจนครบ 1 คน
    while (currentNightCount < 1) {
      let assigned = false;
      for (const assistant of availableAssistants) {
        if (usedAssistants.has(assistant.id)) continue;
