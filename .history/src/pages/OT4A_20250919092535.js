...
const OT4A = ({ userData }) => {
  ...
  useEffect(() => {
    if (monthlyStaff.length > 0 && compensationRates && shiftData) {
  ...
  const fetchMonthlyStaff = async (rates = {}, shiftData = {}) => {
    try {
      ...
      const nursesList = monthlyStaffList.filter(member => nurseRoles.includes(member.role));
      const assistantsList = monthlyStaffList.filter(member => assistantRoles.includes(member.role));
      ...
      const updatedStaff = Array(8).fill().map((_, index) => {
        ...
        if (globalIndex < monthlyStaffList.length) {
          const member = monthlyStaffList[globalIndex];
          // ดึงอัตราค่าจ้างตามตำแหน่ง
          const rate = rates[member.id]?.monthlyOT || '';
  ...
  const updateStaffWithShiftData = async (shiftData = {}) => {
    ...
    const updatedStaff = formData.staff.map((staff, index) => {
      const globalIndex = (currentPage - 1) * staffPerPage + index;
      if (globalIndex < monthlyStaff.length) {
        const member = monthlyStaff[globalIndex];
        ...
        const days = Array(31).fill('').map((_, dayIndex) => {
          ...
          if (dayIndex === 0) {
            ...
            console.log(`Update - shiftColor=${shiftDataItem?.textColor}, oncallColor=${oncallDataItem?.textColor}`);
          }
  ...
  const totalPages = Math.ceil(monthlyStaff.length / staffPerPage);

  // คำนวณจำนวนเจ้าหน้าที่ในหน้าปัจจุบัน
  const startIndex = (currentPage - 1) * staffPerPage;
  const endIndex = Math.min(startIndex + staffPerPage, monthlyStaff.length);
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-4 mb-4">
              ...
              <span className="text-sm text-gray-600">
                หน้า {currentPage} จาก {totalPages} ({startIndex + 1}-{endIndex} จาก {monthlyStaff.length} คน)
              </span>
              <button
                onClick={() => handlePageChange(currentPage + 1)}
