...
const ShiftSchedule = ({ userData }) => {
  ...
  const dateColumns = Array.from({ length: 31 }, (_, i) => {
    const date = i + 1;
    const isWeekend = [0, 6].includes(new Date(2023, 0, date).getDay()); // 0 for Sunday, 6 for Saturday
                 ...
                 <thead className="bg-gradient-to-r from-slate-700 to-slate-800">
                   <tr>
                     ...
                     {dateColumns.map(date => (
                       <th key={date.date} className="border border-slate-600 px-2 py-3 text-xs font-semibold text-white uppercase tracking-wider w-20 text-center">
                         {date.date}
                       </th>
                     ))}
                        ...
                        {index + 1}
                         ...
                         <div className="flex flex-col items-center">
                           ...
                           <span className="text-xs text-slate-600">{member.lastName}</span>
                                               ...
                                               {dateColumns.map(date => (
                          <td key={date.date} className="border border-gray-200 px-2 py-2 text-sm text-gray-900 text-center cursor-pointer hover:bg-blue-50">
                            {editingCell === `${member.id}-${date.date}-top` ? (
                              <input
                                type="text"
                                value={editingValue}
                                onChange={handleCellChange}
                                onKeyDown={(e) => handleKeyDown(e, member.id, date.date, 'top')}
                                onBlur={() => handleCellSave(member.id, date.date, 'top')}
                                className="w-full text-center text-xs border-none outline-none bg-blue-100 rounded"
                                autoFocus
                              />
                            ) : (
                              <div 
                                ...
                                onClick={() => handleCellClick(member.id, date.date, 'top')}
                              >
                                <span className="text-xs text-slate-500">
                                  {scheduleData[`${member.id}-${date.date}-top`] || '-'}
                                </span>
                                <span className="text-xs text-slate-400">
                                  {scheduleData[`${member.id}-${date.date}-bottom`] || '-'}
                                </span>
                              </div>
                            )}
                          </td>
