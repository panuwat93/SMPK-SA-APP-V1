...
const ShiftSchedule = ({ userData }) => {
  ...
  const loadScheduleFromFirebase = (targetDate = currentMonth) => {
    try {
      ...
      console.log(`🔗 Setting up real-time subscription for: ${userData.department}-${monthKey}`);
      
      // ใช้ onSnapshot สำหรับ Real-time updates
      const unsubscribe = onSnapshot(scheduleRef, (scheduleDoc) => {
        console.log(`📡 Real-time update received for ${monthKey}:`, scheduleDoc.exists());
        ...
        setScheduleStatus(scheduleData.status || 'published');
          ...
          console.log('🧹 Clearing cell values for new month');
          setCellValues({});
          setCellStyles({});
          setIsScheduleSaved(false);
  ...
  const changeMonth = async (direction) => {
    ...
    console.log('📅 New month:', newDate.toISOString().substr(0, 7));
    
    // อัปเดตเดือน - Real-time subscription จะจัดการโหลดข้อมูลตารางเวรเอง
    setCurrentMonth(newDate);
    
    // โหลดข้อมูลวันหยุดของเดือนใหม่ทันที
    if (userData?.department) {
      await loadHolidaysFromFirebase(newDate);
      console.log('✅ Holidays loaded for new month');
      // ตารางเวรจะอัปเดตอัตโนมัติผ่าน Real-time subscription ใน useEffect
    }
  };
  ...
  const saveCellValue = async (memberId, dayIndex, rowType = 'shift', value) => {
    ...
    console.log(`🔍 saveCellValue called:`, {
      memberId, dayIndex, rowType, value, cellKey,
        ...
        <div className="flex items-center gap-2">
          ...
          {shiftOptions.filter(option => !['ช', 'บ', 'ด'].includes(option.name)).map(option => (
            <button
              ...
              style={{ 
                backgroundColor: option.name === 'O' ? '#ffffff' : 
                               option.name === 'MB' ? '#16a34a' : 
                               option.name === 'ช*' ? '#ffffff' :
                               option.name === 'VA' ? '#dc2626' :
                               (option.backgroundColor || '#6b7280'),
                color: option.name === 'O' ? '#dc2626' : 
                       option.name === 'MB' ? '#000000' : 
                       option.name === 'ช*' ? '#7c3aed' :
                       option.name === 'VA' ? '#000000' :
                       (option.textColor || '#ffffff'),
                border: (option.name === 'O' || option.name === 'MB' || option.name === 'ช*' || option.name === 'VA') ? '1px solid #d1d5db' : 'none'
              }}
              title={option.name}
            >
              {option.name}
                ...
                {canEditSchedule && (
                  <div className="text-center mt-1">
                    <p className="text-xs sm:text-sm text-gray-600">
                      🔄 บันทึกอัตโนมัติเหมือน Google Sheets
                    </p>
                    {userData?.userType !== 'Admin' && (
