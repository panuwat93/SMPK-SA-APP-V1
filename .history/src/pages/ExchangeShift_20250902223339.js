...
const ExchangeShift = ({ userData }) => {
  ...
  useEffect(() => {
    if (userData?.department) {
      if (isSupervisor) {
        // สำหรับหัวหน้า - ดึงรายการขอแลกทั้งหมด
  ...
  const handleSaveExchange = () => {
    ...
    // บันทึกลง localStorage เพื่อให้หัวหน้าเห็น
    const existingRequests = JSON.parse(localStorage.getItem('exchangeRequests') || '[]');
    const updatedRequests = [exchangeRequest, ...existingRequests];
    localStorage.setItem('exchangeRequests', JSON.stringify(updatedRequests));
  ...
  const fetchAllExchangeRequests = async () => {
    try {
      // TODO: ดึงข้อมูลจาก Firebase
      // ตอนนี้ใช้ข้อมูลจาก localStorage หรือ state ที่บันทึกไว้
      const savedRequests = localStorage.getItem('exchangeRequests');
      if (savedRequests) {
        const parsedRequests = JSON.parse(savedRequests);
        setAllExchangeRequests(parsedRequests);
      } else {
  ...
  const handleApproveRequest = (requestId) => {
    const updatedRequests = allExchangeRequests.map(request => 
      request.id === requestId 
        ? { ...request, status: 'approved' }
        : request
    );
    
    setAllExchangeRequests(updatedRequests);
    
    // บันทึกลง localStorage
    localStorage.setItem('exchangeRequests', JSON.stringify(updatedRequests));
    
    alert('อนุมัติการแลกเวรเรียบร้อย');
  };
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            {allExchangeRequests.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-gray-600">เมื่อเจ้าหน้าที่ส่งคำขอแลกเวร รายการจะแสดงที่นี่</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {allExchangeRequests.map((request) => (
