...
const PreviousSchedule = ({ userData }) => {
  ...
  const fetchSchedules = async () => {
    try {
      ...
      // ลบข้อมูลซ้ำตาม monthKey (เก็บแค่เดือนละ 1 รายการ)
      const uniqueSchedules = [];
      const seenMonths = new Set();
      
      schedulesList.forEach(schedule => {
        if (!seenMonths.has(schedule.monthKey)) {
          seenMonths.add(schedule.monthKey);
                 ...
                 <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-xl border border-blue-200/50 overflow-hidden">
                   ...
                   <div className="p-6 text-center">
                     {(() => {
                       ...
                       if (nurseOverview && nurseOverview.imageData) {
                         return (
                           <img 
                             src={nurseOverview.imageData}
                             alt="ตารางพยาบาล"
                             className="w-64 h-40 object-contain border-2 border-blue-300 rounded-lg mx-auto cursor-pointer hover:border-blue-400 transition-colors"
                             onClick={() => showFullImage({
                               title: `ตารางพยาบาล - ${getMonthName(selectedMonth)}`,
                               type: 'nurses',
                               month: selectedMonth,
                               imageData: nurseOverview.imageData
                             })}
                           />
                         );
                       } else {
                 ...
                 <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-xl border border-green-200/50 overflow-hidden">
                   ...
                   <div className="p-6 text-center">
                     {(() => {
                       ...
                       if (assistantOverview && assistantOverview.imageData) {
                         return (
                           <img 
                             src={assistantOverview.imageData}
                             alt="ตารางผู้ช่วย"
                             className="w-64 h-40 object-contain border-2 border-green-300 rounded-lg mx-auto cursor-pointer hover:border-green-400 transition-colors"
                             onClick={() => showFullImage({
                               title: `ตารางผู้ช่วย - ${getMonthName(selectedMonth)}`,
                               type: 'assistants',
                               month: selectedMonth,
                               imageData: assistantOverview.imageData
                             })}
                           />
                         );
                       } else {
              ...
              <div className="text-center">
                {selectedImage.imageData ? (
                  <img 
                    ...
                    alt="รูปภาพตารางเวร"
                    className="w-full h-auto max-h-96 object-contain border-2 border-gray-300 rounded-lg"
                  />
                ) : (
