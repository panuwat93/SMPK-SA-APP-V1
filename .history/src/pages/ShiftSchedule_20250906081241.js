      ...
      const unsubscribe = onSnapshot(scheduleRef, (scheduleDoc) => {
        ...
        setCellValues(prevCellValues => {
          ...
          Object.keys(newCellValues).forEach(cellKey => {
            ...
            // อัปเดตเฉพาะช่องที่:
            // 1. ไม่เคยแก้ไข หรือ
            // 2. แก้ไขมานานกว่า 3 วินาที และค่าจาก Firebase ต่างจากค่าปัจจุบัน
            if (timeSinceEdit > 3000 || !window.lastCellEditTime?.[cellKey]) {
              mergedValues[cellKey] = newCellValues[cellKey];
            }
            
            console.log(`Cell ${cellKey}: lastEdit=${timeSinceEdit}ms, keeping local=${timeSinceEdit <= 3000}`);
          });
