  ...
  const fetchShiftData = async (selectedMonth = null) => {
    try {
      ...
      if (selectedMonth) {
        ...
        // ใช้เดือนปัจจุบัน แต่ปี 2024
        const currentDate = new Date();
        monthKey = `2024-${String(currentDate.getMonth() + 1).padStart(2, '0')}`;
      }
      
      console.log('Fetching shift data for monthKey:', monthKey);
      const scheduleRef = doc(db, 'shiftSchedules', `${userData.department}-${monthKey}`);
      const scheduleDoc = await getDoc(scheduleRef);
      
      if (scheduleDoc.exists()) {
        const data = scheduleDoc.data();
        console.log('Shift data found:', data);
        setShiftData(data);
        return data;
      } else {
        console.log('No shift data found for monthKey:', monthKey);
        
        // ลองหาใน collection อื่นๆ
      ...
      const updatedStaff = Array(8).fill().map((_, index) => {
        if (index < monthlyStaffList.length) {
          ...
          const days = Array(31).fill('').map((_, dayIndex) => {
            ...
            // กรองเฉพาะเวรสีแดง (ช/บ/ด/MB) และไม่รวม O
