...
const OT4A = ({ userData }) => {
  ...
  const fetchMonthlyStaff = async (rates = {}) => {
    try {
      setLoading(true);
      ...
      const updatedStaff = Array(8).fill().map((_, index) => {
        if (index < monthlyStaffList.length) {
          ...
          const days = Array(31).fill('').map((_, dayIndex) => {
            ...
            let combinedShift = '';
            if (shiftValue && oncallValue) {
              combinedShift = `${shiftValue}/${oncallValue}`;
            } else if (shiftValue) {
              combinedShift = shiftValue;
            } else if (oncallValue) {
              combinedShift = oncallValue;
            }
