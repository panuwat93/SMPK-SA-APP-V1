...
const ShiftSchedule = ({ userData }) => {
  ...
  const [editPermissionEnabled, setEditPermissionEnabled] = useState(false);
  const [visibleNursesStart, setVisibleNursesStart] = useState(0);
  const [visibleAssistantsStart, setVisibleAssistantsStart] = useState(0);
  
  // ฟังก์ชันสำหรับเลื่อนรายชื่อพยาบาล
  const scrollNursesUp = () => {
    if (visibleNursesStart > 0) {
      setVisibleNursesStart(visibleNursesStart - 1);
    }
  };
  
  const scrollNursesDown = () => {
    if (visibleNursesStart < nurses.length - 5) {
      setVisibleNursesStart(visibleNursesStart + 1);
    }
  };
  
  // ฟังก์ชันสำหรับเลื่อนรายชื่อผู้ช่วย
  const scrollAssistantsUp = () => {
    if (visibleAssistantsStart > 0) {
      setVisibleAssistantsStart(visibleAssistantsStart - 1);
    }
  };
  
  const scrollAssistantsDown = () => {
    if (visibleAssistantsStart < assistants.length - 5) {
      setVisibleAssistantsStart(visibleAssistantsStart + 1);
    }
  };
                ...
                <button
                    ...
                    >
                      <span className="text-xs">
                        {editPermissionEnabled ? 'ปิดสิทธิ์การแก้ไข' : 'เปิดสิทธิ์การแก้ไข'}
                      </span>
                </button>
                  </>
                )}
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-xl border border-blue-200/50 overflow-hidden">
                <div className="bg-blue-100 text-blue-800 p-4 border-l-4 border-blue-400">
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl font-semibold">พยาบาล ({nurses.length} คน)</h2>
                    {nurses.length > 5 && (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={scrollNursesUp}
                          disabled={visibleNursesStart === 0}
                          className={`px-3 py-1 rounded text-sm font-medium transition-colors ${
                            visibleNursesStart === 0
                        ...
                        <span className="text-sm">
                          {visibleNursesStart + 1}-{Math.min(visibleNursesStart + 5, nurses.length)} / {nurses.length}
                        </span>
                        <button
                          onClick={scrollNursesDown}
                          disabled={visibleNursesStart >= nurses.length - 5}
                          className={`px-3 py-1 rounded text-sm font-medium transition-colors ${
                            visibleNursesStart >= nurses.length - 5
                              ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                              : 'bg-blue-600 hover:bg-blue-700 text-white'
                          }`}
                        >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {nurses.slice(visibleMembersStart, visibleMembersStart + 5).map((nurse, index) => (
            ...
            {assistants.length > 0 && (viewMode === 'all' || viewMode === 'assistants') && (
              <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-xl border border-blue-200/50 overflow-hidden">
                <div className="bg-green-100 text-green-800 p-4 border-l-4 border-green-400">
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl font-semibold">ผู้ช่วยพยาบาล / ผู้ช่วยเหลือคนไข้ ({assistants.length} คน)</h2>
                    {assistants.length > 5 && (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={scrollMembersUp}
                          disabled={visibleMembersStart === 0}
                          className={`px-3 py-1 rounded text-sm font-medium transition-colors ${
                            visibleMembersStart === 0
                              ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                        ...
                        <span className="text-sm">
                          {visibleMembersStart + 1}-{Math.min(visibleMembersStart + 5, assistants.length)} / {assistants.length}
                        </span>
                        <button
                          onClick={scrollMembersDown}
                          disabled={visibleMembersStart >= assistants.length - 5}
                          className={`px-3 py-1 rounded text-sm font-medium transition-colors ${
                            visibleMembersStart >= assistants.length - 5
                              ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                              : 'bg-green-600 hover:bg-green-700 text-white'
                          }`}
                        >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-xl border-collapse">
                    ...
                    <tbody>
                      {assistants.slice(visibleMembersStart, visibleMembersStart + 5).map((assistant, index) => (
