...
const OT4C2 = ({ userData }) => {
  ...
  const [currentStaffType, setCurrentStaffType] = useState('nurse'); // 'nurse' or 'assistant'
  ...
  useEffect(() => {
    if (dailyStaff.length > 0 && compensationRates && shiftData) {
      fetchDailyStaff(compensationRates, shiftData, {});
    }
  }, [currentPage]);

  // โหลดข้อมูลใหม่เมื่อเปลี่ยนประเภทเจ้าหน้าที่
  useEffect(() => {
    if (dailyStaff.length > 0 && compensationRates && shiftData) {
      fetchDailyStaff(compensationRates, shiftData, {});
    }
  ...
  const fetchDailyStaff = async (rates = {}, currentShiftData = {}, previousShiftData = {}) => {
    try {
      ...
      // แยกพยาบาลและผู้ช่วย
      const nurseRoles = ['พยาบาลวิชาชีพชำนาญการพิเศษ', 'พยาบาลวิชาชีพชำนาญการ', 'พยาบาลวิชาชีพปฏิบัติการ', 'พยาบาลวิชาชีพ'];
      const assistantRoles = ['ผู้ช่วยพยาบาล', 'พนักงานผู้ช่วยเหลือคนไข้'];
      
      const nursesList = dailyStaffList.filter(member => nurseRoles.includes(member.role));
      const assistantsList = dailyStaffList.filter(member => assistantRoles.includes(member.role));
      
      setNurses(nursesList);
      setAssistants(assistantsList);
      
      // อัปเดตฟอร์มด้วยข้อมูลเจ้าหน้าที่รายวัน
      const currentStaffList = currentStaffType === 'nurse' ? nursesList : assistantsList;
      const updatedStaff = Array(8).fill().map((_, index) => {
        ...
        if (globalIndex < currentStaffList.length) {
          ...
          // ดึงอัตราค่าจ้างตามตำแหน่ง (ใช้ค่าเวร(รายวัน/รายเดือน))
          ...
          const days = Array(31).fill('').map((_, dayIndex) => {
            ...
            // ตรวจสอบสีของเวร - ต้องเป็นสีดำ (#000000)
            ...
            // ดึงเฉพาะเวร บ/ด ที่เป็นสีดำ (ค่าเวร ไม่ใช่ OT)
          ...
          // คำนวณจำนวนวันค่าเวร (นับเวรที่ไม่ว่าง)
          const totalShiftDays = days.filter(day => day.trim() !== '').length;
          
          // คำนวณจำนวนเงิน (จำนวนวัน × อัตราตอบแทน)
          const totalAmount = totalShiftDays * (parseFloat(rate) || 0);
          ...
          return {
            ...
            rate: rate,
            days: days,
            totalDays: totalShiftDays.toString(),
  ...
  const updateStaffWithShiftData = async (currentShiftData = {}, previousShiftData = {}) => {
    const currentStaffList = currentStaffType === 'nurse' ? nurses : assistants;
    ...
    const updatedStaff = formData.staff.map((staff, index) => {
      ...
      if (globalIndex < currentStaffList.length) {
        ...
        // คำนวณจำนวนเงิน (จำนวนวัน × อัตราตอบแทน)
        const totalAmount = totalShiftDays * (parseFloat(staff.rate) || 0);
  ...
  const handlePageChange = async (newPage) => {
    ...
    await fetchDailyStaff(compensationRates, shiftData, {});
  };

  // คำนวณจำนวนหน้าทั้งหมด
  const totalPages = Math.ceil(dailyStaff.length / staffPerPage);

  // คำนวณจำนวนเจ้าหน้าที่ในหน้าปัจจุบัน
  const startIndex = (currentPage - 1) * staffPerPage;
  const endIndex = Math.min(startIndex + staffPerPage, dailyStaff.length);
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-4 mb-4">
              ...
              <span className="text-sm text-gray-600">
                หน้า {currentPage} จาก {totalPages} ({startIndex + 1}-{endIndex} จาก {dailyStaff.length} คน)
              </span>
              <button
                onClick={() => handlePageChange(currentPage + 1)}
