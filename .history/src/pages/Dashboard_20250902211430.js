...
const Dashboard = ({ user }) => {
  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      try {
        const teamDoc = await getDoc(doc(db, 'team', 'members'));
        if (teamDoc.exists()) {
  ...
  useEffect(() => {
    const fetchScheduleData = async () => {
      ...
      try {
        ...
        console.log('Fetching schedule for:', docId, 'User:', user.uid);
  ...
  const getStaffShifts = (date) => {
    ...
    // หา memberIndex จาก teamMembers หรือใช้ uid เป็น key
    let memberIndex = -1;
    if (teamMembers.length > 0) {
      ...
      // ถ้า teamMembers ยังว่างเปล่า ให้ลองหาใน schedule data โดยตรง
      const scheduleArray = scheduleData.schedule;
      if (Array.isArray(scheduleArray)) {
        for (let i = 0; i < scheduleArray.length; i++) {
          const memberData = scheduleArray[i];
          if (memberData && memberData.uid === user.uid) {
            memberIndex = i;
            break;
          }
        }
        console.log('Found member index from schedule data:', memberIndex);
      }
    }
    ...
    const memberSchedule = scheduleData.schedule[memberIndex];
    const daySchedule = memberSchedule?.[dayIndex];
    const topShift = daySchedule?.topShift || '';
    const bottomShift = daySchedule?.bottomShift || '';
    
    console.log('Shifts for day', dayIndex, ':', { topShift, bottomShift, memberSchedule, daySchedule });
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="mb-4 p-4 bg-gray-100 rounded-lg text-sm">
                ...
                {scheduleData && (
                  <div>
                    <p>Schedule Array Length: {scheduleData.schedule?.length || 0}</p>
                    <p>First Member UID: {scheduleData.schedule?.[0]?.uid || 'No UID'}</p>
                  </div>
                )}
              </div>
