...
const Settings = ({ userData }) => {
  ...
  const saveExtraDutyOptions = async () => {
    ...
    try {
      ...
      alert('บันทึกตัวเลือกหน้าที่เสริมเรียบร้อยแล้ว');
    } catch (error) {
          ...
          {activeMenu === 'beds' && (
           ...
           {activeMenu === 'extraDuties' && (
             <div className="bg-white rounded-lg shadow-md p-6">
               ...
               <p className="text-gray-600 mb-6">การตั้งค่าสำหรับจัดการหน้าที่เสริม</p>
               
               {/* เพิ่มตัวเลือกหน้าที่เสริมใหม่ */}
               <div className="mb-6">
                 <h3 className="text-lg font-medium text-gray-800 mb-3">เพิ่มตัวเลือกหน้าที่เสริมใหม่</h3>
                 <div className="flex gap-3">
                   <input
                     type="text"
                     value={newExtraDutyOption}
                     onChange={(e) => setNewExtraDutyOption(e.target.value)}
                     placeholder="พิมพ์ชื่อหน้าที่เสริม เช่น ดูแลเครื่องมือ, จัดการเอกสาร, ตรวจสอบอุปกรณ์"
                     className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
               ...
               {/* แสดงตัวเลือกหน้าที่เสริมที่มีอยู่ */}
               <div className="mb-6">
                 <h3 className="text-lg font-medium text-gray-800 mb-3">ตัวเลือกหน้าที่เสริมที่มีอยู่ ({extraDutyOptions.length} รายการ)</h3>
                 {extraDutyOptions.length === 0 ? (
                   <div className="text-center text-gray-500 py-8">
                     ยังไม่มีตัวเลือกหน้าที่เสริม
                   </div>
                 ) : (
                   <div className="space-y-2">
               ...
               <div className="flex justify-end">
                 ...
                 >
                   {savingExtraDuties ? 'กำลังบันทึก...' : extraDutiesSaved ? 'บันทึกแล้ว' : 'บันทึกตัวเลือกหน้าที่เสริม'}
                 </button>
               </div>
             </div>
           )}
