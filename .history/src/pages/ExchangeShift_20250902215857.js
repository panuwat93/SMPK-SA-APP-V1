    ...
    try {
      ...
      if (teamDoc.exists()) {
        ...
        const exchangeableMembers = members.filter(member => {
          // ไม่รวมตัวเอง
          if (member.id === userData.uid) return false;
          ...
          if (userData.role === 'ผู้ช่วยพยาบาล' || userData.role === 'ผู้ช่วยเหลือคนไข้') {
            return member.role === 'ผู้ช่วยพยาบาล' || member.role === 'ผู้ช่วยเหลือคนไข้';
          }
          
          return false;
        });
        setTeamMembers(sameRoleMembers);
      }
    } catch (error) {
