...
const Assignments = ({ userData }) => {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(assignmentsRef, async (assignmentsDoc) => {
        if (assignmentsDoc.exists()) {
          ...
          if (data.lastSavedByAll) {
            setLastSavedBy(data.lastSavedByAll);
          }
        } else {
  ...
  const saveAssignmentsToFirebase = async (assignmentsData = assignments, staffType = 'all') => {
    ...
    try {
      ...
      await setDoc(assignmentsRef, {
        ...
        lastSavedByAll: staffType === 'all' ? `${userData.firstName} ${userData.lastName}` : currentDoc.exists() ? currentDoc.data().lastSavedByAll || '' : ''
      });
      ...
      if (staffType === 'nurses') {
        ...
        setLastSavedBy(`${userData.firstName} ${userData.lastName}`);
      }
    } catch (error) {
  ...
  const saveAssignments = async () => {
    ...
    setSaving(true);
    try {
      await saveAssignmentsToFirebase(assignments, 'all');
      setSaved(true);
    ...
    <div className="assignments-container min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-slate-100 p-4">
      ...
      {canManageAssignments && (
        <div className="space-y-4">
          {/* แสดงข้อมูลผู้บันทึกล่าสุดแยกตามประเภท */}
          <div className="text-center space-y-2">
            ...
            {/* บันทึกทั้งหมดล่าสุด (ถ้ามี) */}
            {lastSavedBy && (
              <div className="inline-flex items-center px-4 py-2 bg-purple-50 border border-purple-200 rounded-lg text-purple-700 text-sm">
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                บันทึกทั้งหมดล่าสุดโดย: {lastSavedBy}
              </div>
            )}
          </div>
          
          {/* ปุ่มบันทึกแยกตามกลุ่ม */}
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            >
              {saving ? 'กำลังบันทึก...' : saved ? 'บันทึกทั้งหมดแล้ว' : 'บันทึกทั้งหมด'}
            </button>
          </div>
        </div>
      )}
