...
const ShiftSchedule = ({ userData }) => {
  ...
  const loadHolidaysFromFirebase = async (targetDate = currentMonth) => {
    try {
      ...
      console.log(`🔍 โหลดข้อมูลวันหยุดเดือน ${month}/${year} (${monthKey})`);
      ...
      if (holidaysDoc.exists()) {
        ...
        console.log(`✅ โหลดข้อมูลวันหยุดสำเร็จ: ${savedHolidays.length} วัน`, savedHolidays);
      } else {
        setHolidays([]);
  ...
  const changeMonth = (direction) => {
    ...
    console.log('📅 เดือนใหม่:', newDate.toLocaleDateString('th-TH', { month: 'long', year: 'numeric' }));
    
    // อัปเดตเดือน
    setCurrentMonth(newDate);
    
    // โหลดข้อมูลวันหยุดของเดือนใหม่ทันที (ไม่ต้องรอ)
    if (userData?.department) {
      console.log('📥 เริ่มโหลดข้อมูลวันหยุดของเดือนใหม่...');
      loadHolidaysFromFirebase(newDate); // ลบ await ออก
    }
  };
