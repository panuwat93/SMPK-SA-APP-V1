...
const ShiftSchedule = ({ userData }) => {
  ...
  const fetchHolidaysFromAPI = async () => {
    ...
    try {
      ...
      // ทดสอบและเลือก API ที่ใช้งานได้
      console.log('เริ่มทดสอบ API วันหยุดราชการ...');
      const workingAPI = await testHolidayAPIs(year);
      
      let holidaysData = null;
      let apiSource = 'fallback';
      
      if (workingAPI) {
        holidaysData = workingAPI.data;
        apiSource = workingAPI.api;
        console.log(`ใช้ API: ${apiSource}`);
      } else {
        console.log('ไม่มี API ที่ใช้งานได้ ใช้ข้อมูลพื้นฐานแทน');
      }
      ...
      if (holidaysData && Array.isArray(holidaysData) && holidaysData.length > 0) {
        ...
        if (formattedHolidays.length > 0) {
          ...
          // ไม่มีข้อมูลวันหยุดในเดือนนี้ ใช้ข้อมูลพื้นฐาน
          console.log('ไม่มีข้อมูลวันหยุดในเดือนนี้ ใช้ข้อมูลพื้นฐานแทน');
          const basicHolidays = getBasicHolidays(year, month);
          setHolidays(basicHolidays);
          await saveHolidaysToFirebase(basicHolidays);
          setHasFetchedHolidays(true);
          setHolidayMessage({ 
            text: `ไม่มีข้อมูลวันหยุดในเดือนนี้ ใช้ข้อมูลพื้นฐาน (${basicHolidays.length} วัน)`, 
            type: 'info' 
          });
        }
      } else {
        // ไม่มีข้อมูลจาก API ใช้ข้อมูลพื้นฐาน
        ...
        setHasFetchedHolidays(true);
        setHolidayMessage({ 
          text: `ไม่สามารถดึงข้อมูลจาก API ได้ ใช้ข้อมูลพื้นฐาน (${basicHolidays.length} วัน)`, 
          type: 'info' 
        });
      }
    } catch (error) {
  ...
  const fetchSavedHolidaysOnly = async () => {
    try {
      ...
      if (holidaysDoc.exists()) {
        const savedHolidays = holidaysDoc.data().holidays || [];
        setHolidays(savedHolidays);
        // ตั้งค่า hasFetchedHolidays เป็น true เพราะมีข้อมูลที่บันทึกไว้แล้ว
        setHasFetchedHolidays(true);
  ...
  const changeMonth = (direction) => {
    ...
    // โหลดข้อมูลวันหยุดจากฐานข้อมูลทันที (ถ้ามี)
    setTimeout(() => {
      if (userData?.department) {
        fetchSavedHolidaysOnly();
      }
    }, 100); // รอให้ currentMonth อัปเดตก่อน
  };
