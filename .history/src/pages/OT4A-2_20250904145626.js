...
const OT4A2 = ({ userData }) => {
  ...
  const loadData = async () => {
    await fetchCompensationRates();
    await fetchMonthlyStaff();
  };
    ...
    try {
      ...
      const updatedStaff = Array(8).fill().map((_, index) => {
        if (index < monthlyStaffList.length) {
          ...
          const rate = compensationRates[member.role]?.night || '';
          return {
            no: (index + 1).toString(),
    ...
    <div className="min-h-screen bg-white p-4">
      <div className="max-w-7xl mx-auto print-content" style={{ width: '297mm', minHeight: '210mm' }}>
        ...
        <div className="text-center mb-8 no-print">
          <h1 className="text-2xl font-bold text-gray-800 mb-4">แบบฟอร์ม OT-4A (2)</h1>
        </div>
        ...
        <div className="bg-white border border-gray-300 p-6">
          ...
          <div className="mb-6 relative">
            ...
            <div className="absolute top-0 right-0">
              <p className="text-sm font-bold">แบบฟอร์ม OT - 4A (2)</p>
            </div>
          </div>
