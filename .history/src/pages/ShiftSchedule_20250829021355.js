...
const ShiftSchedule = ({ userData }) => {
  ...
  // ดึงข้อมูลวันหยุดที่บันทึกไว้เมื่อเปลี่ยนเดือน (เพื่อแสดงสีในคอลัมน์)
  useEffect(() => {
    if (userData?.department) {
      fetchSavedHolidaysOnly();
    }
  ...
  // ทดสอบ API วันหยุดราชการ
  const testHolidayAPIs = async (year) => {
    const apis = [
      {
        name: 'date.nager.at',
      ...
      {
        name: 'calendarific',
        url: `https://calendarific.com/api/v2/holidays?api_key=8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c&country=TH&year=${year}&type=national`,
        headers: { 'Accept': 'application/json' }
      },
      {
        name: 'holidayapi',
        url: `https://holidayapi.com/v1/holidays?country=TH&year=${year}&key=8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c`,
        headers: { 'Accept': 'application/json' }
      }
    ];
    ...
    for (const api of apis) {
      try {
        console.log(`ทดสอบ API: ${api.name}`);
        ...
        if (response.ok) {
          ...
          if (responseText.trim()) {
            ...
            console.log(`✅ ${api.name} ใช้งานได้:`, data);
            return { api: api.name, data: data };
          }
        }
        console.log(`❌ ${api.name} ไม่สามารถใช้งานได้: ${response.status}`);
      } catch (error) {
        console.log(`❌ ${api.name} เกิดข้อผิดพลาด:`, error);
      }
    }
  ...
  const fetchHolidaysFromAPI = async () => {
    ...
    try {
      ...
      // ลองใช้ API วันหยุดราชการของประเทศไทย (API แรก)
      let holidaysData = null;
      let apiSource = 'date.nager.at';
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (responseText.trim()) {
            ...
            console.log('ดึงข้อมูลจาก date.nager.at สำเร็จ');
          }
        }
      } catch (error) {
        console.log('API แรกล้มเหลว, ลอง API ที่สอง:', error);
      }
      
      // ถ้า API แรกล้มเหลว ลองใช้ API ที่สอง (Calendarific - ใช้งานได้จริง)
      if (!holidaysData) {
        try {
          const response2 = await fetch(`https://calendarific.com/api/v2/holidays?api_key=8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c&country=TH&year=${year}&type=national`, {
            method: 'GET',
            headers: {
              'Accept': 'application/json'
            }
          ...
          if (response2.ok) {
            ...
            if (responseText2.trim()) {
              const data2 = JSON.parse(responseText2);
              holidaysData = data2.response?.holidays || [];
              apiSource = 'calendarific';
              console.log('ดึงข้อมูลจาก calendarific สำเร็จ');
            }
          }
        } catch (error) {
          console.log('API ที่สองล้มเหลว:', error);
        }
      }
      
      // ถ้า API ที่สองล้มเหลว ลองใช้ API ที่สาม (Holiday API - ใช้งานได้จริง)
      if (!holidaysData) {
        try {
          const response3 = await fetch(`https://holidayapi.com/v1/holidays?country=TH&year=${year}&key=8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c8c`, {
            method: 'GET',
            headers: {
              'Accept': 'application/json'
            }
          ...
          if (response3.ok) {
            ...
            if (responseText3.trim()) {
              const data3 = JSON.parse(responseText3);
              holidaysData = data3.holidays || [];
              apiSource = 'holidayapi';
              console.log('ดึงข้อมูลจาก holidayapi สำเร็จ');
            }
          }
        } catch (error) {
          console.log('API ที่สามล้มเหลว:', error);
        }
      }
      ...
      if (holidaysData && Array.isArray(holidaysData) && holidaysData.length > 0) {
        ...
        const monthHolidays = holidaysData.filter(holiday => {
          ...
          // รองรับรูปแบบวันที่จาก API ต่างๆ
          if (holiday.date) {
            holidayDate = new Date(holiday.date);
          } else if (holiday.date?.iso) {
            holidayDate = new Date(holiday.date.iso);
          } else if (holiday.date?.datetime) {
            holidayDate = new Date(holiday.date.datetime);
          }
        ...
        const formattedHolidays = monthHolidays.map(holiday => {
          ...
          // รองรับชื่อวันหยุดจาก API ต่างๆ
        ...
        setHasFetchedHolidays(true);
        setHolidayMessage({ 
          text: `ดึงข้อมูลวันหยุดราชการสำเร็จ (${formattedHolidays.length} วัน) จาก ${apiSource}`, 
          type: 'success' 
        });
        console.log('วันหยุดในเดือนนี้:', formattedHolidays);
      } else {
        console.log('ไม่สามารถดึงข้อมูลจาก API ได้ ใช้ข้อมูลพื้นฐานแทน');
        setHolidayMessage({ 
  ...
  // ดึงเฉพาะข้อมูลวันหยุดที่บันทึกไว้ (ไม่ดึงจาก API)
  const fetchSavedHolidaysOnly = async () => {
    try {
      const year = currentMonth.getFullYear();
      const month = currentMonth.getMonth() + 1;
      const monthKey = `${year}-${month.toString().padStart(2, '0')}`;
      ...
      if (holidaysDoc.exists()) {
        const savedHolidays = holidaysDoc.data().holidays || [];
        setHolidays(savedHolidays);
        // ไม่ตั้งค่า hasFetchedHolidays เป็น true เพราะนี่เป็นการดึงข้อมูลที่บันทึกไว้ ไม่ใช่การดึงจาก API
        console.log('ดึงข้อมูลวันหยุดที่บันทึกไว้:', savedHolidays);
      } else {
        // ไม่มีข้อมูลที่บันทึกไว้
        setHolidays([]);
        setHasFetchedHolidays(false); // แสดงว่าต้องดึงข้อมูล
        console.log('ไม่มีข้อมูลวันหยุดที่บันทึกไว้');
      }
    } catch (error) {
      console.error('Error fetching saved holidays:', error);
      setHolidays([]);
  ...
  const getBasicHolidays = (year, month) => {
    ...
    if (month === 2 || month === 3) {
      ...
      if (month === 3) {
        holidays.push({ date: 15, name: 'วันมาฆบูชา', type: 'public' });
      }
    }
    ...
    if (month === 5 || month === 6) {
      // ประมาณเดือนพฤษภาคม-มิถุนายน
      if (month === 6) {
        holidays.push({ date: 15, name: 'วันวิสาขบูชา', type: 'public' });
      }
    }
    ...
    if (month === 10) {
      holidays.push({ date: 23, name: 'วันปิยมหาราช', type: 'public' });
    }
    
    // วันลอยกระทง (ขึ้น 15 ค่ำ เดือน 12)
    if (month === 11) {
      // ประมาณเดือนพฤศจิกายน
      holidays.push({ date: 15, name: 'วันลอยกระทง', type: 'public' });
    }
    ...
    if (month === 12) {
      holidays.push({ date: 10, name: 'วันรัฐธรรมนูญ', type: 'public' });
    }
          ...
          <div className="grid grid-cols-3 items-center mb-4">
            <div className="flex justify-start gap-3">
              <button
                ...
                disabled={loadingHolidays || hasFetchedHolidays}
