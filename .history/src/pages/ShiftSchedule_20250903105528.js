      ...
      for (const member of allMembers) {
        ...
        if (requiredOT > 0) {
          ...
          days.forEach((day, dayIndex) => {
            // ตรวจสอบแถวบน (shift)
            const topCellKey = `${member.id}-${dayIndex}-shift`;
            const topValue = cellValues[topCellKey];
            const topStyle = cellStyles[topCellKey];
            
            // ตรวจสอบแถวล่าง (oncall)
            const bottomCellKey = `${member.id}-${dayIndex}-oncall`;
          ...
          // เรียงลำดับตาม priority (แถวบนก่อน) และสุ่ม
          availableShifts.sort((a, b) => {
            if (a.priority !== b.priority) {
              return a.priority - b.priority;
            }
            return Math.random() - 0.5; // สุ่ม
          });
          
          // เลือกเวรตามจำนวน OT ที่ต้องการ
          const shiftsToAssign = availableShifts.slice(0, requiredOT);
          ...
          shiftsToAssign.forEach(shift => {
            ...
            addToHistory({
              ...
              oldColor: prev[shift.cellKey]?.textColor || '#000000',
              newColor: '#ff0000'
            });
          });
              ...
              <div className="flex justify-start gap-3">
                {canEditSchedule && (
                  <>
                    ...
                    >
                      <span className="text-xs">{isScheduleSaved ? 'บันทึกแล้ว (แบบร่าง)' : 'บันทึกแบบร่าง'}</span>
                    </button>
