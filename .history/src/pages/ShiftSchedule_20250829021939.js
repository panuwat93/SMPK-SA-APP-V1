...
const ShiftSchedule = ({ userData }) => {
  ...
  // ดึงเฉพาะข้อมูลวันหยุดที่บันทึกไว้ (ไม่ดึงจาก API) - ทำงานเร็ว
  const fetchSavedHolidaysOnly = async () => {
    try {
      ...
      console.log(`โหลดข้อมูลวันหยุดเดือน ${month}/${year} จากฐานข้อมูล...`);
      ...
      if (holidaysDoc.exists()) {
        ...
        setHolidays(savedHolidays);
        setHasFetchedHolidays(true);
        console.log(`✅ โหลดข้อมูลวันหยุดสำเร็จ: ${savedHolidays.length} วัน`);
      } else {
        setHolidays([]);
        setHasFetchedHolidays(false);
        console.log(`❌ ไม่มีข้อมูลวันหยุดในเดือน ${month}/${year}`);
      }
    } catch (error) {
