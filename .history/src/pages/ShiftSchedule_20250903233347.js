...
const ShiftSchedule = ({ userData }) => {
  ...
  const [visibleMembersStart, setVisibleMembersStart] = useState(0);
  
  // ฟังก์ชันสำหรับเลื่อนรายชื่อ
  const scrollMembersUp = () => {
    if (visibleMembersStart > 0) {
      setVisibleMembersStart(visibleMembersStart - 1);
    }
  };
  
  const scrollMembersDown = () => {
    if (visibleMembersStart < teamMembers.length - 5) {
      setVisibleMembersStart(visibleMembersStart + 1);
    }
  };
