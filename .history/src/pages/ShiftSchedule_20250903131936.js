  ...
  const autoAssignAssistantShifts = async () => {
    ...
    try {
      ...
      for (let dayIndex = 0; dayIndex < days.length; dayIndex++) {
        ...
        // ตรวจสอบว่าวันนี้มีเวรที่ต้องการจัดครบแล้วหรือไม่ (เช้า 2, บ่าย 2, ดึก 2)
        ...
        assistants.forEach(assistant => {
          ...
          // นับเวรเช้า (ช, ด ในแถวบน)
          if (topValue === 'ช' || topValue === 'ด') {
            morningCount++;
          }
          
          // นับเวรบ่าย (บ ในแถวล่าง)
          if (bottomValue === 'บ') {
            afternoonCount++;
          }
          
          // นับเวรดึก (ด ในแถวล่าง)
          if (bottomValue === 'ด') {
            nightCount++;
          }
        });
        
        // ถ้ามีเวรครบแล้ว (เช้า 2, บ่าย 2, ดึก 2) ให้ข้ามวันนี้
        if (morningCount >= 2 && afternoonCount >= 2 && nightCount >= 2) {
          console.log(`วันที่ ${day.day} มีเวรครบแล้ว (เช้า:${morningCount}, บ่าย:${afternoonCount}, ดึก:${nightCount}) ข้าม`);
          continue;
        }
        
        console.log(`วันที่ ${day.day} ต้องจัดเวรเพิ่ม (เช้า:${morningCount}/2, บ่าย:${afternoonCount}/2, ดึก:${nightCount}/2)`);
        ...
        // จัดเวรทั้งหมดสำหรับวันนี้ (เช้า 2, บ่าย 2, ดึก 2)
        const assignedShifts = assignAllShiftsForDay(dayIndex, shiftCounts);
        
        console.log(`วันที่ ${day.day}: จัดเวร ${assignedShifts.length} เวร (เช้า:${morningCount}, บ่าย:${afternoonCount}, ดึก:${nightCount})`);
        
        // อัพเดทจำนวนเวรหลังจากจัดเวร
        let newMorningCount = 0, newAfternoonCount = 0, newNightCount = 0;
        assistants.forEach(assistant => {
          const topCellKey = `${assistant.id}-${dayIndex}-top`;
          const bottomCellKey = `${assistant.id}-${dayIndex}-bottom`;
          const topValue = cellValues[topCellKey] || '';
        ...
        console.log(`วันที่ ${day.day} หลังจัดเวร: เช้า:${newMorningCount}, บ่าย:${newAfternoonCount}, ดึก:${newNightCount}`);
        
        // ถ้ายังไม่ครบตามจำนวนที่ต้องการ ให้จัดเวรเพิ่ม
        if (newMorningCount < 2 || newAfternoonCount < 2 || newNightCount < 2) {
          console.log(`วันที่ ${day.day} ยังไม่ครบตามจำนวนที่ต้องการ ต้องจัดเวรเพิ่ม`);
          // จัดเวรเพิ่มจนครบ
          assignRemainingShifts(dayIndex, newMorningCount, newAfternoonCount, newNightCount, shiftCounts);
        }
      }
