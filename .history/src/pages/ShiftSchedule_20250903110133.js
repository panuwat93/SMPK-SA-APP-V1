        ...
        if (requiredOT > 0) {
          ...
          days.forEach((day, dayIndex) => {
            ...
            if (topValue && ['ช', 'บ', 'ด', 'MB'].includes(topValue)) {
              console.log(`วันที่ ${dayIndex}: แถวบน = ${topValue}, สี = ${topStyle?.textColor || 'ไม่มีสี'}`);
            }
            if (bottomValue && ['ช', 'บ', 'ด', 'MB'].includes(bottomValue)) {
              console.log(`วันที่ ${dayIndex}: แถวล่าง = ${bottomValue}, สี = ${bottomStyle?.textColor || 'ไม่มีสี'}`);
            }
