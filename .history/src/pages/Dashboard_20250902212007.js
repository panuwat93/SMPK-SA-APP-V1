...
const Dashboard = ({ user }) => {
  ...
  const getStaffShifts = (date) => {
    ...
    console.log('Getting shifts for day:', dayIndex, 'Date:', date.getDate());
    
    // หา memberIndex จาก teamMembers
    let memberIndex = -1;
    if (teamMembers.length > 0) {
      memberIndex = teamMembers.findIndex(member => member.uid === user.uid);
      console.log('Found member index from teamMembers:', memberIndex);
    }
    
    if (memberIndex === -1) {
      console.log('Member not found, user uid:', user.uid);
      return { topShift: '', bottomShift: '' };
    }
    
    const memberId = teamMembers[memberIndex].id;
    const memberUid = teamMembers[memberIndex].uid;
    console.log('Member ID:', memberId, 'Member UID:', memberUid);
    
    // ลองใช้ทั้ง memberId และ memberUid
    let memberSchedule = scheduleData.schedule[memberId];
    if (!memberSchedule) {
      memberSchedule = scheduleData.schedule[memberUid];
      console.log('Using UID instead of ID for schedule lookup');
    }
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="mb-4 p-4 bg-gray-100 rounded-lg text-sm">
                ...
                <p>User Department: {user?.department || 'No Department'}</p>
                <p>Team Members: {teamMembers.length}</p>
                <p>Schedule Data: {scheduleData ? 'Loaded' : 'Not loaded'}</p>
                <p>Current Month: {currentDate.toLocaleDateString('th-TH', { year: 'numeric', month: 'long' })}</p>
                {scheduleData && (
                  <div>
                    <p>Schedule Data: {scheduleData.schedule ? 'Available' : 'Not available'}</p>
                    <p>Schedule Members: {Object.keys(scheduleData.schedule || {}).length}</p>
                    <p>Schedule Keys: {Object.keys(scheduleData.schedule || {}).join(', ')}</p>
                  </div>
                )}
                {teamMembers.length > 0 && (
                  <div>
                    <p>My Member ID: {teamMembers.find(m => m.uid === user?.uid)?.id || 'Not found'}</p>
                    <p>My Member UID: {teamMembers.find(m => m.uid === user?.uid)?.uid || 'Not found'}</p>
                    <p>My Name: {teamMembers.find(m => m.uid === user?.uid)?.firstName} {teamMembers.find(m => m.uid === user?.uid)?.lastName}</p>
                    <p>All Team UIDs: {teamMembers.map(m => m.uid).join(', ')}</p>
                  </div>
                )}
              </div>
                      ...
                      {generateCalendar().map((week, weekIndex) => (
                        <tr key={weekIndex} className="border-b border-gray-100">
                          {week.map((date, dayIndex) => {
                            ...
                            const { topShift, bottomShift } = getStaffShifts(date);
                              ...
                              >
                                <div className="flex flex-col h-full">
                                  ...
                                  {topShift && topShift.trim() !== '' && (
                                    <div className={`text-xs px-1 py-0.5 rounded mb-1 ${getShiftColor(topShift)}`}>
                                  ...
                                  {bottomShift && bottomShift.trim() !== '' && (
                                    <div className={`text-xs px-1 py-0.5 rounded ${getShiftColor(bottomShift)}`}>
