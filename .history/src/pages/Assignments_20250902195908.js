...
const Assignments = ({ userData }) => {
  // ตรวจสอบสิทธิ์การมอบหมายงาน
  const isSupervisor = userData?.role === 'หัวหน้าหน่วยงาน';
  const hasManagePermission = isSupervisor || userData?.permissions?.manageAssignments;
  ...
  if (!hasManagePermission) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-slate-100 p-4 flex items-center justify-center">
        <div className="text-center bg-white/90 backdrop-blur-sm rounded-xl shadow-xl p-8 border border-red-200/50 max-w-md">
          ...
          <p className="text-gray-600 mb-6">
            คุณไม่มีสิทธิ์ในการจัดการตารางมอบหมายงาน<br />
            กรุณาติดต่อหัวหน้าหน่วยงานเพื่อขอสิทธิ์
          </p>
          <div className="flex items-center justify-center space-x-2 text-sm text-gray-500">
            <span>❌</span>
            <span>สิทธิ์การมอบหมายงาน</span>
          </div>
        </div>
      </div>
    );
