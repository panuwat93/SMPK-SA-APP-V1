    ...
    try {
      ...
      const updatedStaff = Array(8).fill().map((_, index) => {
        ...
        if (globalIndex < currentStaffList.length) {
          ...
          // ดึงอัตราค่าจ้างตามตำแหน่ง (ใช้เวร OT รายวัน)
          const rate = rates[member.id]?.dailyOT || '';
          
          // ดึงข้อมูลเวรสำหรับ 31 วัน
          const days = Array(31).fill('').map((_, dayIndex) => {
            ...
            if (isShiftRed && shiftValue !== 'O') {
              // ถ้าเป็น MB ให้แสดงเป็น ช
              validShiftValue = shiftValue === 'MB' ? 'ช' : shiftValue;
            }
            
            if (isOncallRed && oncallValue !== 'O') {
              // ถ้าเป็น MB ให้แสดงเป็น ช
              validOncallValue = oncallValue === 'MB' ? 'ช' : oncallValue;
            }
                   ...
                   <tr key={staffIndex}>
                     ...
                     {staff.days.slice(20, 31).map((day, dayIndex) => (
                       <td key={dayIndex} className="border border-gray-400 p-1 w-6">
                         <input 
                           type="text" 
                           className="w-full text-center border-none outline-none text-red-600 font-medium"
                           value={day}
