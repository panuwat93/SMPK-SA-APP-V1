  ...
  const fetchShifts = async (memberId, date) => {
    ...
    try {
      ...
      if (scheduleDoc.exists()) {
        ...
        console.log('Cell styles keys:', Object.keys(cellStyles));
        
        // ดึงเวรของตัวเอง - ใช้ key แบบ {uid}-{dayIndex}-{top/bottom}
        const myTopCellKey = `${userData.uid}-${dayIndex}-top`;
        const myBottomCellKey = `${userData.uid}-${dayIndex}-bottom`;
        const myTopShift = schedule[myTopCellKey] || '';
        const myBottomShift = schedule[myBottomCellKey] || '';
        
        console.log('My cell keys:', { myTopCellKey, myBottomCellKey });
        console.log('My shifts:', { top: myTopShift, bottom: myBottomShift });
        
        setMyShifts({ top: myTopShift, bottom: myBottomShift });
        
        // ดึงสไตล์ของตัวเอง
        setMyShiftStyles({ 
          top: cellStyles[myTopCellKey] || {}, 
          bottom: cellStyles[myBottomCellKey] || {} 
        });
        
        // ดึงเวรของคนที่จะแลก - ใช้ key แบบ {uid}-{dayIndex}-{top/bottom}
        const otherTopCellKey = `${memberId}-${dayIndex}-top`;
        const otherBottomCellKey = `${memberId}-${dayIndex}-bottom`;
        const otherTopShift = schedule[otherTopCellKey] || '';
        const otherBottomShift = schedule[otherBottomCellKey] || '';
        
        console.log('Other cell keys:', { otherTopCellKey, otherBottomCellKey });
        console.log('Other shifts:', { top: otherTopShift, bottom: otherBottomShift });
        
        setOtherShifts({ top: otherTopShift, bottom: otherBottomShift });
        
        // ดึงสไตล์ของคนที่จะแลก
        setOtherShiftStyles({ 
          top: cellStyles[otherTopCellKey] || {}, 
          bottom: cellStyles[otherBottomCellKey] || {} 
        });
      } else {
