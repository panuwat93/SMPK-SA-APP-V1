...
const ShiftSchedule = ({ userData }) => {
  ...
  // บันทึกตารางเวรก่อนแลกลง Firebase
  const saveScheduleBeforeExchange = async () => {
    try {
      const monthKey = `${currentMonth.getFullYear()}-${String(currentMonth.getMonth() + 1).padStart(2, '0')}`;
      const beforeExchangeRef = doc(db, 'schedulesBeforeExchange', `${userData.department}-${monthKey}`);
      
      // สร้างข้อมูลตารางเวรก่อนแลก
      const scheduleData = {};
      ...
      // บันทึกลง Firebase ใน collection 'schedulesBeforeExchange'
      await setDoc(beforeExchangeRef, {
        department: userData.department,
        monthKey: monthKey,
        ...
        type: 'beforeExchange',
        isReadOnly: true // แก้ไขไม่ได้
      });
      
      // แจ้งเตือนสำเร็จ
      setHolidayMessage({ 
        text: 'บันทึกตารางเวรก่อนแลกสำเร็จ', 
        type: 'success' 
      });
      
      console.log('บันทึกตารางเวรก่อนแลกสำเร็จ');
    } catch (error) {
      console.error('Error saving schedule before exchange:', error);
      setHolidayMessage({ 
        text: 'เกิดข้อผิดพลาดในการบันทึกตารางเวรก่อนแลก', 
        type: 'error' 
      });
    }
  };
      ...
      <div className="w-full">
        ...
        <div className="mb-6 bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-gray-200">
              ...
              <div className="flex justify-start gap-3">
                ...
                <button
                  onClick={() => saveScheduleBeforeExchange()}
                  className="px-3 py-1.5 rounded text-sm font-medium transition-colors bg-gray-600 hover:bg-gray-700 text-white"
                >
                  <span className="text-xs">บันทึกตารางเวรก่อนแลก</span>
                </button>
