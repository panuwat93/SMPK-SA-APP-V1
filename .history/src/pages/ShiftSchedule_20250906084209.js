...
const ShiftSchedule = ({ userData }) => {
  ...
  const saveCellStyles = async () => {
    try {
      ...
      // คงสถานะเดิมไว้เพื่อให้ทุกคนเห็นการอัปเดต
      if (isAlreadyPublished) {
        updateData.status = 'published';
        updateData.publishedAt = new Date();
        updateData.publishedBy = userData.username;
  ...
  const saveCellValue = async (memberId, dayIndex, rowType = 'shift', value) => {
    ...
    try {
      ...
      // คงสถานะเดิมไว้ หรือตั้งเป็น published ถ้าเคยส่งออกแล้ว
      if (isAlreadyPublished) {
        updateData.status = 'published';
        updateData.publishedAt = new Date();
        updateData.publishedBy = userData.username;
      } else if (currentStatus) {
        // คงสถานะเดิมไว้ (อาจจะเป็น draft หรืออะไรก็ตาม)
        updateData.status = currentStatus;
      } else {
        // ถ้าไม่เคยมีสถานะ ให้ตั้งเป็น published เพื่อให้ทุกคนเห็น
        updateData.status = 'published';
        updateData.publishedAt = new Date();
        updateData.publishedBy = userData.username;
      }

      console.log('📤 Auto-saving with status:', updateData.status);
      await updateDoc(scheduleRef, updateData);
      console.log('✅ Auto-save successful - should trigger real-time sync');
    } catch (error) {
      console.error('Error saving schedule:', error);
    }
  };
