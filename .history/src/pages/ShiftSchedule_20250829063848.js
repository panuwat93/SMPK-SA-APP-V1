...
const ShiftSchedule = ({ userData }) => {
  ...
  const saveOverviewImage = async () => {
    try {
      ...
      // บันทึกทั้ง 2 ตารางพร้อมกัน (พยาบาล + ผู้ช่วย)
      const batch = writeBatch(db);
      
      // บันทึกตารางพยาบาล
      const nurseOverviewRef = doc(db, 'schedulesBeforeExchange', `${userData.department}-${monthKey}-nurses`);
      batch.set(nurseOverviewRef, {
        department: userData.department,
        monthKey: monthKey,
        overviewType: 'nurses',
        savedAt: new Date(),
      ...
      // บันทึกตารางผู้ช่วย
      const assistantOverviewRef = doc(db, 'schedulesBeforeExchange', `${userData.department}-${monthKey}-assistants`);
      batch.set(assistantOverviewRef, {
        department: userData.department,
        monthKey: monthKey,
        overviewType: 'assistants',
        savedAt: new Date(),
        savedBy: userData.username,
        type: 'overview',
        tableHTML: tableHTML,
      ...
      // บันทึกทั้ง 2 รายการพร้อมกัน
      await batch.commit();
      
      setHolidayMessage({ 
        text: 'บันทึกภาพรวมพยาบาลและผู้ช่วยสำเร็จ', 
        type: 'success' 
      });
      
      setShowOverviewModal(false);
      console.log('บันทึกภาพรวมพยาบาลและผู้ช่วยสำเร็จ');
