...
const Dashboard = ({ user }) => {
  ...
  const getStaffShifts = (date) => {
    if (!scheduleData?.schedule || !user?.uid) return { topShift: '', bottomShift: '' };
    ...
    if (memberIndex === -1) return { topShift: '', bottomShift: '' };
    
    const topShift = scheduleData.schedule[memberIndex]?.[dayIndex]?.topShift || '';
    const bottomShift = scheduleData.schedule[memberIndex]?.[dayIndex]?.bottomShift || '';
    
    return { topShift, bottomShift };
  };

  // ฟังก์ชันแปลงเวรเป็นสี
  const getShiftColor = (shift) => {
    switch (shift) {
      case 'ช': return 'bg-blue-100 text-blue-800';
      case 'บ': return 'bg-green-100 text-green-800';
      case 'ด': return 'bg-purple-100 text-purple-800';
                            ...
                            return (
                              ...
                              >
                                <div className="flex flex-col h-full">
                                  ...
                                  {/* เวรแถวบน */}
                                  {topShift && (
                                    <div className={`text-xs px-2 py-1 rounded mb-1 ${getShiftColor(topShift)}`}>
                                  ...
                                  {/* เวรแถวล่าง */}
                                  {bottomShift && (
                                    <div className={`text-xs px-2 py-1 rounded ${getShiftColor(bottomShift)}`}>
