...
const ShiftSchedule = ({ userData }) => {
  ...
  const addShiftToSelectedCell = (shiftName) => {
    ...
    cellsToUpdate.forEach(cellKey => {
      if (cellKey) {
        ...
        // เก็บข้อมูลสไตล์ลงใน cellStyles (ถ้ามีการเลือกสีหรือขนาด)
        if (textColor !== '#000000' || fontSize !== 14) {
          setCellStyles(prev => ({
            ...prev,
            [cellKey]: {
              ...prev[cellKey],
  ...
  const countTotalShiftsForMember = (memberId) => {
    ...
    days.forEach((day, dayIndex) => {
      ...
      const bottomValue = cellValues[bottomCellKey] || '';
      if (bottomValue && bottomValue.trim() !== '') {
        totalShifts++;
      }
    });
  ...
  const getShiftStyle = (shiftName, cellKey = null) => {
    ...
    if (shiftOption) {
      return {
        color: customStyle.textColor || shiftOption.textColor || '#000000',
                      ...
                      {nurses.map((nurse, index) => (
                        <React.Fragment key={nurse.id}>
                          ...
                          <tr className="h-8">
                            ...
                            <td rowSpan="2" className="border border-gray-300 px-3 py-2 text-center text-sm font-medium bg-gray-50 align-middle">
                              {calculateOTForMember(nurse.id)}
                            </td>
                            <td rowSpan="2" className="border border-gray-300 px-3 py-2 text-center text-sm font-medium bg-gray-50 align-middle">
                              {(() => {
                                const totalShifts = countTotalShiftsForMember(nurse.id);
                                return totalShifts > 0 ? totalShifts * 100 : 0; // ตัวอย่าง: 100 บาทต่อเวร
                              })()}
                            </td>
                          </tr>
                      ...
                      {assistants.map((assistant, index) => (
                        <React.Fragment key={assistant.id}>
                          ...
                          <tr className="h-8">
                            ...
                            <td rowSpan="2" className="border border-gray-300 px-3 py-2 text-center text-sm font-medium bg-gray-50 align-middle">
                              {calculateOTForMember(assistant.id)}
                            </td>
                            <td rowSpan="2" className="border border-gray-300 px-3 py-2 text-center text-sm font-medium bg-gray-50 align-middle">
                              {(() => {
                                const totalShifts = countTotalShiftsForMember(assistant.id);
                                return totalShifts > 0 ? totalShifts * 100 : 0; // ตัวอย่าง: 100 บาทต่อเวร
                              })()}
                            </td>
                          </tr>
                              ...
                              {Array.from({ length: new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 0).getDate() }, (_, dayIndex) => {
                                ...
                                return (
                                  ...
                                  >
                                    <span 
                                      className="text-sm font-medium px-1 py-0.5 rounded"
                                      style={getShiftStyle(cellValue, `${member.id}-${dayIndex}-top`)}
                                    >
                                      {cellValue}
                                    </span>
                                  </td>
