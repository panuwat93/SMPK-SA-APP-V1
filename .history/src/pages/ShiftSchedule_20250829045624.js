...
const ShiftSchedule = ({ userData }) => {
  ...
  const saveOverviewImage = async () => {
    try {
      ...
      setHolidayMessage({ 
        text: 'บันทึกภาพรวมสำเร็จ', 
        type: 'success' 
      });
      
      setShowOverviewModal(false);
      console.log('บันทึกภาพรวมสำเร็จ');
    } catch (error) {
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="mb-4 p-3 bg-yellow-100 rounded-lg text-sm">
                  ...
                  <p>จำนวนสมาชิกทั้งหมด: {teamMembers.length} คน</p>
                  <p>ประเภทที่เลือก: {overviewType}</p>
                  <p>จำนวนข้อมูลเวร: {Object.keys(cellValues).length} รายการ</p>
                  <p>สมาชิกที่ filter ได้: {teamMembers.filter(member => {
                    if (overviewType === 'nurses') {
                      return member.position === 'พยาบาล';
                    } else {
                      return member.position === 'ผู้ช่วยพยาบาล' || member.position === 'พนักงานผู้ช่วยเหลือคนไข้';
                    }
                  }).length} คน</p>
                  
                  {/* แสดงรายละเอียดสมาชิก */}
                  <div className="mt-2 p-2 bg-white rounded border">
                    <p><strong>รายละเอียดสมาชิก:</strong></p>
                    {teamMembers.map((member, index) => (
                      <p key={member.id} className="text-xs">
                        {index + 1}. {member.firstName} {member.lastName} - ตำแหน่ง: {member.position}
                      </p>
                    ))}
                  </div>
                  ...
                  {/* แสดงข้อมูลเวร */}
                  <div className="mt-2 p-2 bg-white rounded border">
                    <p><strong>ข้อมูลเวร (cellValues):</strong></p>
                    <p className="text-xs">Keys: {Object.keys(cellValues).join(', ') || 'ไม่มี'}</p>
                  </div>
                </div>
                  ...
                  <table className="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                          ...
                          const filteredMembers = teamMembers.filter(member => {
                            console.log(`Checking member: ${member.firstName} ${member.lastName}, position: ${member.position}`);
                            
                            if (overviewType === 'nurses') {
                              const isNurse = member.position === 'พยาบาล';
                              console.log(`Is nurse? ${isNurse}`);
                              return isNurse;
                            } else {
                              const isAssistant = member.position === 'ผู้ช่วยพยาบาล' || member.position === 'พนักงานผู้ช่วยเหลือคนไข้';
