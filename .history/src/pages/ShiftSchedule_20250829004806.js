                        ...
                        <React.Fragment key={nurse.id}>
                          ...
                          <tr className="h-8">
                            ...
                            {days.map((day, dayIndex) => {
                              const cellKey = `${nurse.id}-${dayIndex}`;
                              ...
                              return (
                                <td 
                                  ...
                                  onClick={() => startEditing(nurse.id, dayIndex)}
                                >
                                  {isEditing ? (
                                    <input
                                      ref={inputRef}
                                      ...
                                      onKeyDown={(e) => handleKeyDown(e, nurse.id, dayIndex)}
                                      onBlur={() => saveCellValue(nurse.id, dayIndex, cellValue)}
                      ...
                      {assistants.map((assistant, index) => (
                        <React.Fragment key={assistant.id}>
                          ...
                          <tr className="h-8">
                            ...
                            {days.map((day, dayIndex) => {
                              const cellKey = `${assistant.id}-${dayIndex}`;
                              const isEditing = editingCell === cellKey;
                              const cellValue = cellValues[cellKey] || '';
                              ...
                              return (
                                <td 
                                  key={dayIndex} 
                                  className={`border border-gray-300 px-1 py-1 text-center cursor-pointer hover:bg-blue-50 ${day.isWeekend ? 'bg-yellow-50' : ''}`}
                                  onClick={() => startEditing(assistant.id, dayIndex)}
                                >
                                  {isEditing ? (
                                    <input
                                      type="text"
                                      ...
                                      onKeyDown={(e) => handleKeyDown(e, assistant.id, dayIndex)}
                                      onBlur={() => saveCellValue(assistant.id, dayIndex, cellValue)}
                                      className="w-full h-full text-center text-sm border-none outline-none bg-transparent"
                                      autoFocus
                                    />
                                  ) : (
