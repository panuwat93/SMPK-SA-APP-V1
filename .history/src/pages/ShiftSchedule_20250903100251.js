...
const ShiftSchedule = ({ userData }) => {
  ...
  const saveCellValue = async (memberId, dayIndex, rowType = 'shift', value) => {
    ...
    try {
      ...
      console.log('💾 บันทึกเซลล์เดียว:', {
        memberId,
        dayIndex,
        rowType,
        value,
        monthKey,
        path: `${memberId}.${dayIndex}.${rowType}`
      });

      // ใช้ setDoc แทน updateDoc เพื่อสร้างเอกสารใหม่ถ้ายังไม่มี
      await setDoc(scheduleRef, {
        [`${memberId}.${dayIndex}.${rowType}`]: value
      }, { merge: true }); // merge: true เพื่อไม่ให้เขียนทับข้อมูลเดิม
    } catch (error) {
      console.error('Error saving schedule:', error);
    }
  };
      ...
      <div className="w-full">
        ...
        <div className="mb-6 bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-gray-200">
              ...
              <div className="flex justify-start gap-3">
                ...
                <button
                  onClick={() => {
                    console.log('🔍 Debug ข้อมูลปัจจุบัน:', {
                      cellValues: cellValues,
                      cellStyles: cellStyles,
                      currentMonth: currentMonth,
                      monthKey: `${currentMonth.getFullYear()}-${String(currentMonth.getMonth() + 1).padStart(2, '0')}`,
                      isScheduleSaved: isScheduleSaved,
                      autoSaveStatus: autoSaveStatus
                    });
                    alert('ดูข้อมูล Debug ใน Console (F12)');
                  }}
            ...
            <div className="flex justify-between items-center mb-4">
              <div className="text-sm text-gray-600">
                ...
                <span className="ml-4 font-medium">ข้อมูลเซลล์:</span> {Object.keys(cellValues).length} เซลล์
                <span className="ml-4 font-medium">ข้อมูลสไตล์:</span> {Object.keys(cellStyles).length} สไตล์
              </div>
