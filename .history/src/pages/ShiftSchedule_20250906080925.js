  ...
  const loadScheduleFromFirebase = (targetDate = currentMonth) => {
    try {
      ...
      const unsubscribe = onSnapshot(scheduleRef, (scheduleDoc) => {
        ...
        // เฉพาะครั้งแรกที่โหลด หรือเมื่อไม่มีการแก้ไขค้างอยู่
        setCellValues(prevCellValues => {
          ...
          // ถ้ามีการแก้ไขค้างอยู่ ให้ merge อย่างระมัดระวัง
          const mergedValues = { ...newCellValues };
          
          // คงค่าที่เพิ่งแก้ไขไว้ (เช่น จาก toolbar) โดยตรวจสอบเวลา
          const now = Date.now();
          Object.keys(prevCellValues).forEach(cellKey => {
            // ถ้าแก้ไขมาแล้วภายใน 2 วินาที ให้ใช้ค่าเดิม
            const lastEditTime = window.lastCellEditTime?.[cellKey] || 0;
            if (now - lastEditTime < 2000) {
              mergedValues[cellKey] = prevCellValues[cellKey];
            }
          });
