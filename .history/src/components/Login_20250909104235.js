...
const Login = ({ onSwitchToSignup, onLoginSuccess }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ถ้า username มี @ แสดงว่าเป็น email แล้ว
      if (username.includes('@')) {
        email = username;
      } else {
        // ลองหา user ใน Firestore ก่อนด้วย username
        try {
          ...
          if (!querySnapshot.empty) {
            // เจอ user แล้ว ใช้ email จาก Firestore
            const userData = querySnapshot.docs[0].data();
            email = userData.email;
          } else {
            // ไม่เจอใน Firestore ใช้แบบเดิม
            email = `${username}@hospital.com`;
          }
        } catch (firestoreError) {
          // ถ้า query Firestore ไม่ได้ ใช้แบบเดิม
          email = `${username}@hospital.com`;
        }
      }
      ...
      if (error.code === 'auth/user-not-found') {
        ...
        setError('ลองเข้าสู่ระบบผิดหลายครั้ง กรุณารอสักครู่แล้วลองใหม่');
